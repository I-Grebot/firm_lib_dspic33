...
// fSCK = FCY /(PPRE*SPRE)
// Desired baudrate is 4MHz

// fSCK = FCY / 8
#define CC2500_SPI_PPRE 0b10  //  4:1
#define CC2500_SPI_SPRE 0b110 //  2:1
//#define CC2500_SPI_SPRE 0b101 //  8:1
...
// Number of times a given channel is tested to check its CCA value.
#define CC2500_CHANNEL_SCAN_RATE 5000


/*
 * CC2500 Procedures handling
 * --------------------------
 */

// Collection of available CC2500 procedures
typedef enum {
    CC2500_PROCEDURE_CHAN_RX,   // Change channel and go to RX
    CC2500_PROCEDURE_RX,        // Go to RX
    CC2500_PROCEDURE_FAST_TX    // Go to TX mode with Freq. synth ON (fast mode)
} cc2500_proc_t;

// Associated type holding our FSM structure
typedef struct cc2500_proc_fsm cc2500_proc_fsm_t;

// Function prototype for procedure calls
typedef void (*cc2500_proc_f) (cc2500_proc_fsm_t* fsm);

// Finite State Machine content
struct cc2500_proc_fsm {
    cc2500_proc_f cur_state; // Current FSM state
    cc2500_proc_t proc;      // Procedure beeing executed
    uint8_t strobe_occured;  // Flag indicating that strobe was already sent
    uint8_t chan;            // Channel value for procedure requiring its programming
};

// Prototypes of the different procedure calls
void cc2500_proc_state_ready(cc2500_proc_fsm_t* fsm);    // Ready -- procedure is beeing initiated
void cc2500_proc_state_idle(cc2500_proc_fsm_t* fsm);     // Idle
void cc2500_proc_state_channel(cc2500_proc_fsm_t* fsm);  // Change channel
void cc2500_proc_state_cal(cc2500_proc_fsm_t* fsm);      // Calibration
void cc2500_proc_state_flush_rx(cc2500_proc_fsm_t* fsm); // RX-flush
void cc2500_proc_state_rx(cc2500_proc_fsm_t* fsm);       // RX mode
void cc2500_proc_state_fstxon(cc2500_proc_fsm_t* fsm);   // Fast TX mode
void cc2500_proc_state_finish(cc2500_proc_fsm_t* fsm);   // Finish -- procedure is finished
...
// System routines
uint8_t cc2500_do_procedure(cc2500_proc_t _proc, uint8_t _param);
